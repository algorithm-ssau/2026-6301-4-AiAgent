...
Приложение захватывает экран в реальном времени, обнаруживает бутылки алкоголя с помощью нейросети YOLO и рисует поверх них чёрные прямоугольники через прозрачный click-through оверлей. Работает на Windows и Linux (Hyprland/X11).
...
> Для запуска необходим файл `Models/best.onnx`. Если его нет — обучите модель (см. ниже) или запросите у участника 4.
