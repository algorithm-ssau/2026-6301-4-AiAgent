...
1. `CaptureBackend` — абстрактный класс (шаблон) с методами:
   - `start()` — инициализация
   - `grab() -> np.ndarray` — вернуть текущий кадр в формате BGR
   - `stop()` — завершение
   - `get_monitor_rect() -> dict` — вернуть `{"top", "left", "width", "height"}`

2. `ScreenCapturer` — обёртка, которая сама решает, какой бэкенд использовать:
   - Если `os.environ.get("XDG_SESSION_TYPE") == "wayland"` → `WaylandBackend`
   - Иначе → `MssBackend`
   - Снаружи выглядит так же: `start()`, `grab()`, `stop()`, `monitor_rect`

**`Core/backends/mss_backend.py`** — класс `MssBackend`:
- Использует библиотеку `mss` для захвата экрана
- `__init__(monitor_index=1, fps_limit=20)` — монитор и ограничение FPS
- В `grab()` — захватить кадр через `mss`, конвертировать из BGRA в BGR через `cv2.cvtColor`, вернуть numpy array
- Ограничение FPS: замерять время через `time.perf_counter()`, если кадр
  захвачен слишком быстро — подождать через `time.sleep()`
- Если указан несуществующий монитор — бросить `ValueError` с понятным сообщением
- Статический метод `list_monitors() -> List[dict]` — вернуть список мониторов
  без вызова `start()`, нужен GUI для заполнения выпадающего списка:
  ```python
  @staticmethod
  def list_monitors() -> list:
      with mss.mss() as sct:
          return list(sct.monitors[1:])  # [0] — все мониторы вместе, пропускаем
  ```
...
#### Часть 2 — `Overlay/base.py` (сделать сразу после запуска обучения, Фаза 1)
...
Сразу отправить этот файл остальным (нужен Участнику 5).
